...
export function initTokenObserver(): () => void {
  return subscribe<{ sessionId: string; amount: number }>(EventType.TOKEN_PURCHASED, async event => {
    ...
    await supabase.from('token_balances').upsert({ session_id: sessionId, balance: current + amount }, { onConflict: 'session_id' });
  });
}
